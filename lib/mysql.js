...
//readORwrite :: DBConf -> String -> DBConf
  readORwrite = R.curry (function (conf, string) {
    const regex = /(?:insert |update |delete )/igm;
    if (string.search(regex) != -1) {
      if (R.isNil (R.prop ("writer", conf))) { return conf; }
      return R.prop ("writer", conf);
    } else {
      if (R.isNil (R.prop ("reader", conf))) { return conf; }
      return R.prop ("reader", conf);
    }
  });
